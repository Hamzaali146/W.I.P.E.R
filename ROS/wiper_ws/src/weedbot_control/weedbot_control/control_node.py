...
class ControlNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            f"working area: +/-({self.working_area_x_half_mm:.1f},"
            f"{self.working_area_y_half_mm:.1f}) mm [homography frame], "
            f"margin={self.working_area_safety_margin_mm:.1f} mm "
            f"-> accept |x|<={self.working_area_x_half_mm - self.working_area_safety_margin_mm:.1f}, "
            f"|y|<={self.working_area_y_half_mm - self.working_area_safety_margin_mm:.1f}"
        )
